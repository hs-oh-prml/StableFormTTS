...
class NoamSchedule(NoneSchedule):
    ...
    def step(self, num_updates):
        if num_updates < self.warmup_updates:
            weight = num_updates / self.warmup_updates
        else:
            weight = self.warmup_updates**0.5 * num_updates**-0.5

        constant_lr = self.constant_lr
        self.lr = constant_lr * weight
        for param_group in self.optimizer.param_groups:
            param_group["lr"] = self.lr
        return self.lr


class GlowTTSSchedule(NoneSchedule):
    def __init__(self, optimizer, lr, warmup_updates, hidden_channels):
        self.optimizer = optimizer
        self.constant_lr = self.lr = lr
        self.warmup_updates = warmup_updates
    ...
    def step(self, num_updates):
        weight = np.power(self.hidden_channels, -0.5) * np.min(
            [
                np.power(num_updates, -0.5),
                num_updates * np.power(self.warmup_updates, -1.5),
            ]
        )
        constant_lr = self.constant_lr
        self.lr = constant_lr * weight
